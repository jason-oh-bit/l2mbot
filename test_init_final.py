...
class mainCog(commands.Cog): 
	...
	async def deleBossInput_(self, ctx):
		...
		if ctx.message.channel.id == basicSetting[7]:
			...
			for i in range(bossNum):
				tmp_msg = msg
				if i==4 or i==10 or i==14 or i==20 or i==31 or i==32 or i==38 or i==39 or i==40 :
					if len(tmp_msg) > 3 :
						if tmp_msg.find(':') != -1 :
							chkpos = tmp_msg.find(':')
							...
							now2 = datetime.datetime.now() + datetime.timedelta(hours = int(basicSetting[0]))
							tmp_now = datetime.datetime.now() + datetime.timedelta(hours = int(basicSetting[0]))
							tmp_now = tmp_now.replace(hour=int(hours1+8), minute=int(minutes1))
						else:
					...
					if len(tmp_msg) > 3 :
						if tmp_msg.find(':') != -1 :
							...
							now2 = datetime.datetime.now() + datetime.timedelta(hours = int(basicSetting[0]))
							tmp_now = datetime.datetime.now() + datetime.timedelta(hours = int(basicSetting[0]))
							tmp_now = tmp_now.replace(hour=int(hours1+14), minute=int(minutes1))
						else:
							chkpos = len(tmp_msg)-2
							hours1 = tmp_msg[chkpos-2:chkpos]
							minutes1 = tmp_msg[chkpos:chkpos+2]
